...
namespace yolo {
    ...
    /// Layout is NCHW (batch=1 implicit), stored as [C, H, W] or [C] etc.
    struct GPUTensor {
        ...
        /// Convenience: access the underlying WGPUBuffer handle.
        [[nodiscard]] threepp::WgpuBuffer& buffer() { return *buf; }
        [[nodiscard]] const threepp::WgpuBuffer& buffer() const { return *buf; }
    };

    /// Create a GPU tensor with a storage buffer (compute read/write, not CPU-readable).
    ...
    /// Create a GPU tensor with a storage buffer that also supports CPU readback (CopySrc).
    inline GPUTensor makeReadbackTensor(threepp::WgpuRenderer& r, std::initializer_list<uint32_t> shape) {
        GPUTensor t;
        t.shape = shape;
        uint32_t n = 1;
        for (auto s : shape) n *= s;
    ...
    /// Create a GPU tensor for packed-f16 weights (two f16 per u32, bytes = numel*2).
    /// Shape is still stored in logical f32-element terms. The buffer is sized
    /// for numel/2 u32s (assumes numel is even).
    inline GPUTensor makeF16WeightTensor(threepp::WgpuRenderer& r, const std::vector<uint32_t>& shape) {
        GPUTensor t;
        t.shape = shape;
        uint32_t n = 1;
        for (auto s : shape) n *= s;
        size_t byteCount = std::max<size_t>(n, 2u) * 2u;   // 2 bytes per f16, round up to 4
    ...
    /// Create a GPU tensor from a given shape vector.
