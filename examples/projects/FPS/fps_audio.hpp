...
// Kill confirm: a deeper double knock with a low noise tail.
...
// Player-hit cue: body thump under a falling tone with a breathy layer.
...
// One footstep on hard flooring: pitch-dropping heel thump + band-passed scuff.
std::vector<float> synthStep(uint32_t seed, int sr = 44100) {
    ...
    const float scuffAt = rf(0.008f, 0.018f);
...
// Spent-brass tink: two short ringing modes with fast decay, seeded variants.
std::vector<float> synthTink(uint32_t seed, int sr = 44100) {
    std::mt19937 r(seed);
    auto rf = [&](float a, float b) { return a + (b - a) * std::uniform_real_distribution<float>(0.f, 1.f)(r); };
    const int n = sr * 8 / 100;
    std::vector<float> s(n);
    const float f1 = rf(3800.f, 5200.f);
    const float f2 = f1 * rf(1.4f, 1.7f);
    for (int i = 0; i < n; ++i) {
        const float t = static_cast<float>(i) / sr;
        const float ring = std::sin(2.f * math::PI * f1 * t) * 0.7f + std::sin(2.f * math::PI * f2 * t) * 0.4f;
        s[i] = ring * std::exp(-t * rf(55.f, 75.f));
    }
    return normalized(std::move(s), 0.35f);
}

// A pooled, retriggerable sound: round-robins a few voices so rapid fire
// overlaps instead of cutting itself off; voices may hold different variants.
struct Sound {
...
// Like Sound, but spatialised: playAt() drops the source at a world position.
// Used for enemy gunfire so it pans + attenuates relative to the camera.
...
struct SoundBank {
    ...
    Sound shot, empty, reload, hit, kill, hurt, step, metal, tink;
    PositionalSound enemyShot;
    bool ok = false;

    void init(Object3D& attachTo) {
        try {
            const fs::path dir = fs::temp_directory_path() / "threepp_fps_sounds";
            ...
            // Real submachine-gun sample for the gun; the rest stay procedural.
            ...
            std::vector<Spec> specs{
                    ...
                    {"kill", {synthKill(41), synthKill(42)}, &kill, 4, {}},
                    {"hurt", {synthHurt()}, &hurt, 2, {}},
                    {"step", {synthStep(1), synthStep(2), synthStep(3), synthStep(4)}, &step, 4, {}},
                    {"tink", {synthTink(7), synthTink(8), synthTink(9)}, &tink, 4, {}}};
            ...
            for (auto& sp : specs) {
                ...
                if (!sp.file.empty()) {
                    paths.push_back(sp.file);
                } else {
                    for (size_t k = 0; k < sp.variants.size(); ++k) {
            ...
            // enemy rifle fire: spatialised so it reads directionally
            {
                std::string p = fs::exists(gunFile) ? gunFile : (dir / "shot0.wav").string();
                if (!fs::exists(p)) {
                    threepp::audio::writeWav(p, synthShot());
                }
                for (int i = 0; i < 4; ++i) {
                    auto a = std::make_unique<PositionalAudio>(*listener, p);
                    a->setVolume(0.5f);
                    a->setDistanceModel(PositionalAudio::DistanceModel::Inverse);
                    a->setMinDistance(8.f);
                    a->setRolloffFactor(0.5f);
                    enemyShot.voices.push_back(std::move(a));
                }
            }
            ok = true;
