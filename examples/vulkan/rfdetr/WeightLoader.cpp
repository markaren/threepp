...
namespace {
    ...
    T readLE(std::ifstream& f) {
        ...
        if (!f) throw std::runtime_error("rfdetr::parseWeightBinary: unexpected end of file");
        return v;
    }
...
Weights parseWeightBinary(const std::string& path) {
    ...
    if (!f) throw std::runtime_error("rfdetr::parseWeightBinary: cannot open '" + path + "'");

    char magic[4]{};
    f.read(magic, 4);
    if (!f || std::strncmp(magic, "YOLO", 4) != 0)
        throw std::runtime_error("rfdetr::parseWeightBinary: bad magic in '" + path + "'");

    uint32_t version = readLE<uint32_t>(f);
    if (version != 1)
        throw std::runtime_error("rfdetr::parseWeightBinary: unsupported version "
    ...
    for (uint32_t t = 0; t < numTensors; ++t) {
        ...
        if (!f) throw std::runtime_error("rfdetr::parseWeightBinary: failed reading tensor name");
        ...
        if (dataBytes % sizeof(float) != 0)
            throw std::runtime_error("rfdetr::parseWeightBinary: non-float4 data size for '"
                                     + name + "'");

        std::vector<float> data(dataBytes / sizeof(float));
        f.read(reinterpret_cast<char*>(data.data()), dataBytes);
        if (!f) throw std::runtime_error("rfdetr::parseWeightBinary: failed reading data for '"
                                         + name + "'");
