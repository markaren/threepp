...
//   [--denoise 0|1] [--dlss 0|1] [--taa 0|1] [--fixed-dt]
...
int main(int argc, char** argv) {
    ...
    // --dlss 0 turns off BOTH upscalers (DLSS, here DLAA, and FSR, which
    // setDlss(false) would otherwise hand the frame to), so the built-in TAA
    // resolve runs. --taa 0 (with --dlss 0) then turns the primary's TAA off
    // too: the written frame is the shade output itself, no history.
    ...
    int seqN = 0;
    int optAnimate = 1, optRestir = 1, optDenoise = 1, optDlss = 1, optTaa = 1;
    bool fixedDt = false;
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--taa" && i + 1 < argc) optTaa = std::atoi(argv[++i]);
