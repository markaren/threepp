...
std::shared_ptr<Mesh> emissiveBall(const Color& c, float intensity, float r = 0.20f) {
    auto mat = MeshStandardMaterial::create({
            {"color", c}, {"emissive", c},
            {"emissiveIntensity", intensity}, {"roughness", 1.f}});
...
void buildCeilingLights(Scene& scene) {
    ...
    struct Strip { float z; Color col; float intensity; };
    const std::array<Strip, 4> strips = {{
        {-8.0f, Color(1.f,  0.92f, 0.70f), 6.f},  // warm white
        {-1.5f, Color(0.25f, 0.55f, 1.f),  6.f},  // cool blue
        { 4.0f, Color(1.f,  0.52f, 0.08f), 6.f},  // amber
        { 9.5f, Color(0.22f, 0.90f, 0.38f), 6.f}, // green
    }};
...
int main() {
    ...
    int   fpsFrames = 0;
    int   renderMode = 1;// 0=Raytracer 1=PathTracer 2=Raster
    bool  raster     = false;
    float orbitTime  = 0.f;
    ...
    ImguiFunctionalContext ui(canvas, renderer, [&] {
        ...
        ImGui::TextColored({1.f, 1.f, 0.f, 1.f}, "Mode: %s", modeNames[renderMode]);
        ImGui::Text("FPS: %.1f", fps);
        if (!raster) ImGui::Text("Frames accumulated: %d", pathTracer.frameCount());
        ImGui::Separator();
    ...
    canvas.animate([&] {
        ...
        // Analytical spot lights only in non-PT modes
        spot1->visible = (renderMode != 1);
        spot2->visible = (renderMode != 1);

        controls.update();

        if (!raster) pathTracer.render(scene, camera);
