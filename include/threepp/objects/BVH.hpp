...
namespace threepp {
    ...
    private:
        ...
        // Extracts triangles from geometry
        void extractTriangles(const BufferGeometry& geom) {
            triangles.clear();
            geometry = &geom;
        ...
        bool trianglesIntersect(const Triangle& a, const Triangle& b) const {
    ...
    public:
        ...
        [[nodiscard]] static bool intersects(const BVH& b1, const Matrix4& thisMatrix, const BVH& b2, const Matrix4& otherMatrix) {
            if (!b1.root || !b2.root) return false;

            // Create transformed copies of the bounding boxes
            Box3 thisRootBox = b1.root->boundingBox.clone().applyMatrix4(thisMatrix);
            Box3 otherRootBox = b2.root->boundingBox.clone().applyMatrix4(otherMatrix);
            ...
            std::function<bool(const BVHNode*, const BVHNode*)> testNodes = [&b1, &b2, &thisMatrix, &otherMatrix, &testNodes](const BVHNode* nodeA, const BVHNode* nodeB) -> bool {
                // Transform bounding boxes for this test
                Box3 boxA = nodeA->boundingBox.clone().applyMatrix4(thisMatrix);
                Box3 boxB = nodeB->boundingBox.clone().applyMatrix4(otherMatrix);
                ...
                if (nodeA->left == nullptr && nodeA->right == nullptr &&
                    ...
                    for (int idxA : nodeA->triangleIndices) {
                        ...
                        Vector3 a = triA.a().clone().applyMatrix4(thisMatrix);
                        Vector3 b = triA.b().clone().applyMatrix4(thisMatrix);
                        Vector3 c = triA.c().clone().applyMatrix4(thisMatrix);
                        Triangle worldTriA(a, b, c);

                        for (int idxB : nodeB->triangleIndices) {
                            // Transform triangle B to world space
                            Triangle triB = b2.triangles[idxB];
                            Vector3 d = triB.a().clone().applyMatrix4(otherMatrix);
                            Vector3 e = triB.b().clone().applyMatrix4(otherMatrix);
                            Vector3 f = triB.c().clone().applyMatrix4(otherMatrix);
