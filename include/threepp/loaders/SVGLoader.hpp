...
namespace threepp {
    ...
    private:
        ...
        */
        ...
        static unsigned int pointsToStrokeWithBuffers(
    ...
    unsigned int SVGLoader::pointsToStrokeWithBuffers(
            std::vector<Vector2> points,
            const SVGLoader::Style& style,
            unsigned int arcDivisions,
            float minDistance,
    ...
    std::vector<SVGLoader::SVGData> SVGLoader::load(const pugi::xml_node& node) {
    ...
    void SVGLoader::parseNode(const pugi::xml_node& node, SVGLoader::Style style) {
    ...
    float SVGLoader::parseFloatWithUnits(float value, const std::string& theUnit) const {
        ...
        if (theUnit == "px" && defaultUnit != "px") {
            ...
            scale = unitConversion.at("in").at(defaultUnit) / defaultDPI;

        } else {

            scale = unitConversion.at(theUnit).at(defaultUnit);

            if (scale < 0) {

                // Conversion scale to pixels

                scale = unitConversion.at(theUnit).at("in") * defaultDPI;
            }
        }
    ...
    float SVGLoader::parseFloatWithUnits(const std::string& str) const {
    ...
    std::optional<Matrix3> SVGLoader::getNodeTransform(const pugi::xml_node& node) {
    ...
    Matrix3 SVGLoader::parseNodeTransform(const pugi::xml_node& node) const {
    ...
    ShapePath SVGLoader::parseRectNode(const pugi::xml_node& node) const {
    ...
    ShapePath SVGLoader::parsePolygonNode(const pugi::xml_node& node) const {
    ...
    ShapePath SVGLoader::parsePolylineNode(const pugi::xml_node& node) const {
    ...
    ShapePath SVGLoader::parseCircleNode(const pugi::xml_node& node) const {
    ...
    ShapePath SVGLoader::parseEllipseNode(const pugi::xml_node& node) const {
    ...
    ShapePath SVGLoader::parseLineNode(const pugi::xml_node& node) const {
    ...
    SVGLoader::Style SVGLoader::parseStyle(const pugi::xml_node& node, SVGLoader::Style style) const {
    ...
    ShapePath SVGLoader::parsePathNode(const pugi::xml_node& node) {
