...
namespace threepp {
     ...
     * from TracedRasterVisionSensor. What the cloud carries differs by backend:
     * on a raster backend, position and distance are populated from the
     * cube-face depth read and the remaining `LidarReturn` fields are left at
     * sentinel defaults (intensity = 0, normal = 0, hitInstanceId = -1),
     * because the raster pipeline has no material or geometry data. On a
     * `VulkanRenderer` the returns carry the tracer's intensity / normal /
     * hitInstanceId; sentinel returns (misses, fog scatter) and near-clip hits
     * are dropped to match the raster output shape.
     *
     ...
     */
    class LidarSensor: public TracedRasterVisionSensor<LidarReturn> {
    ...
    private:
        ...
        // TracedRasterVisionSensor's hooks: the cube/beam pattern, the six-face
        // depth render + unproject raster pass, and the sentinel/shell filter
        // that shapes the tracer's returns like the raster ones.
        std::unique_ptr<PathTracedLidarSensor> createTracedBackend() override;
        void rasterScan(Renderer& renderer, Scene& scene, std::vector<LidarReturn>& cloud) override;
        bool collectTraced(PathTracedLidarSensor& backend, VulkanRenderer& renderer, std::vector<LidarReturn>& cloud) override;
