...
namespace threepp {
     ...
     *
     * Aiming: beams go down the sensor's LOCAL -Z (camera convention), and
     * lookAt() honours that — the sensor overrides
     * usesCameraLookAtConvention(), so lookAt(target) turns the beams toward
     * the target exactly as it would for a Camera. (Historically the sensor
     * got the plain-Object3D convention — +Z toward the target, beams aimed
     * exactly backwards, an empty cloud as the only symptom — and callers
     * compensated with lookAt(2*position - target). Such mirror-point call
     * sites now aim backwards and must lookAt(target) directly.)
     *
