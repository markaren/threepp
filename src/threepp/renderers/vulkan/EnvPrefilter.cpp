...
namespace threepp::vulkan {
    ...
    Image2D EnvPrefilter::buildPmrem(uint32_t w, uint32_t h,
        ...
        // HDRI sun extraction (CPU, one-shot per env upload). The detector and
        // the sun-clamped copy it produces now live in common/EnvSunExtract.cpp
        // so the GL raster path runs the SAME detector on the SAME pixels — two
        // detectors would light the two backends differently, which is exactly
        // the colour-temperature mismatch the shared helper removes. Behaviour
        // here is unchanged: mips 1..N-1 prefilter from `clamped`, mip 0 keeps
        // the original pixels.
        std::vector<float> clamped;// sun-clamped prefilter source (empty = none)
        bool sunFound = false;
        if (extractSun && outSun) {
            sunFound = extractEnvSun(w, h, pixels, clamped, *outSun);
        }
