...
WgpuGeometries::WgpuGeometries(WgpuState& state)
    : state_(state) {}

std::vector<float> WgpuGeometries::buildInterleavedVertexData(BufferGeometry* geometry, uint32_t count) {
    ...
    // Packed into uv.x for dashed line rendering in Wgpu
...
void WgpuGeometries::storeAttributeVersions(BufferGeometry* geometry, GeometryBuffers& gb) {
...
bool WgpuGeometries::geometryNeedsUpdate(BufferGeometry* geometry, const GeometryBuffers& gb) {
    if (geometry->hasAttribute("position") &&
...
bool WgpuGeometries::indexNeedsUpdate(BufferGeometry* geometry, const GeometryBuffers& gb) {
    return geometry->getIndex() && geometry->getIndex()->version > gb.indexVersion;
}

GeometryBuffers& WgpuGeometries::getOrCreateGeometryBuffers(BufferGeometry* geometry) {
    auto id = geometry->id;
...
WireframeBuffers& WgpuGeometries::getOrCreateWireframeBuffers(BufferGeometry* geometry) {
    auto id = geometry->id;
