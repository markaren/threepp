...
namespace threepp::dawn {
    ...
    std::vector<WGPUBindGroupLayoutEntry> DawnPipelines::buildBindGroupLayoutEntries(uint64_t features) const {
        ...
        // Binding 32-33: environment map (cube)
    ...
    PipelineEntry& DawnPipelines::getOrCreatePipeline(uint64_t features,
        ...
        if (!entry.shader) {
            std::cerr << "DawnPipelines: Failed to create shader module for features 0x"
        ...
        if (!entry.pipeline) {
            std::cerr << "DawnPipelines: Failed to create render pipeline for features 0x"
                      << std::hex << features << std::dec << std::endl;
        }
    ...
    CustomPipelineEntry& DawnPipelines::getOrCreateCustomPipeline(
            ShaderMaterial* sm,
            WGPUTextureFormat surfaceFormat,
            uint32_t sampleCount) {
    ...
    void DawnPipelines::invalidateAll() {
    ...
    void DawnPipelines::onMaterialDispose(unsigned int materialId) {
