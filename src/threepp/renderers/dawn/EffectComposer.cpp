...
EffectComposer::EffectComposer(DawnRenderer& renderer)
...
std::vector<unsigned char> EffectComposer::readRGBPixels() {
    ...
    return dawn::readRGBPixels(pimpl_->device, pimpl_->queue, pimpl_->finalOutputTexture, pimpl_->width, pimpl_->height);
}
