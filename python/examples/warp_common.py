...
loops -- live here so the examples can be read for the physics alone. The
Warp-free pieces (command line, the ffmpeg pipe, a material helper) live in
`demo_common.py` and are re-exported from here.
...
# because the hello-world demos and the renderer-only kits (fireworks.py,
# drone_rig.py) share them too. noqa: F401 - re-export.
from demo_common import (Encoder, cli_arg, encode_png_sequence,  # noqa: F401
                         encode_rgbe, find_ffmpeg, parse_size,
