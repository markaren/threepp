...
gl_buffer_id + wp.RegisteredGLBuffer) and the sim copies positions and colors
into them device-to-device — nothing crosses host memory and Python never
touches the bytes. --no-direct (or a CPU device) falls back to the tier-1
path: pinned host mirrors, GPU/CPU pipelining, colors every 3rd frame.
...
def step(pos: wp.array(dtype=wp.vec3),
    ...
    # animated curl-noise turbulence (4th dimension is time)
    s = wp.rand_init(7)
    a += wp.curlnoise(s, wp.vec4(p[0] * FREQ, p[1] * FREQ, p[2] * FREQ, t * 0.15)) * TURB
    ...
    col[i] = (base * (1.0 - h) + wp.vec3(1.0, 0.85, 0.65) * h) * BRIGHT
...
def enable_host_path():
    global DIRECT, pos_host, col_host, pos_view, col_view
    DIRECT = False
...
def gpu_advance(buf):
    ...
    wp.launch(step, dim=N, device=device,
              inputs=[pos, vel, col, DT, sim_time, blast, frame_no])
...
def step_frame_direct():
    """Sim + device-to-device copy into the renderer's own VBOs. Zero host traffic."""
    global sim_time, frame_no
    ph = sim_time - NOVA_T0
    blast = NOVA_AMP * math.exp(-(ph % NOVA_PERIOD) / NOVA_TAU) if ph >= 0.0 else 0.0
    t0 = time.perf_counter()
    wp.launch(step, dim=N, device=device,
              inputs=[pos, vel, col, DT, sim_time, blast, frame_no])
    dst = reg_pos.map(dtype=wp.vec3, shape=(N,))
    wp.copy(dst, pos)
    reg_pos.unmap()
    dst = reg_col.map(dtype=wp.vec3, shape=(N,))
    wp.copy(dst, col)
    reg_col.unmap()
