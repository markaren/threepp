...
simulation never stops.
    ...
    python warp_sailboat.py --film t.mp4 --preview --shots 5 --no-spray  # the A/B
...
# ---- hull spray --------------------------------------------------------------
#  The AIRBORNE half of what the hull does to the water. The sea half is already
#  here -- the exclusion footprint, the Kelvin wake, the foam stamps -- and none
#  of it ever leaves the surface. This is the water that does: a fixed pool of
#  ballistic droplets thrown off the bow shoulders, flown under gravity and
#  quadratic air drag through the film's own wind, and killed where they rejoin
#  the sea, which is also where they stamp foam. The splash goes back into the
#  water it came out of.
#
#  A REAL PARTICLE SIM AND NOT A DENSITY VOLUME, on purpose. What the eye reads
#  as spray is parallax and SIZE SPREAD: hundreds of independently ballistic
#  specks at a hundred different sizes, the fine stuff blown flat downwind and
#  the heavy stuff punching its own arc through the same air. A froxel volume
#  has neither -- it has an average, and an average of spray is haze.
#
#  Two gates decide how much there is, and both are physics rather than a
#  per-shot dial: her SPEED (a bow wave's energy goes as u^2) and how deep the
#  forefoot is BURIED. A flat noon reach at hull speed barely wets the topsides;
#  the same boat at the same speed in a storm sea buries the bow every third
#  wave and throws the whole sheet. That is why one set of numbers has to serve
#  the calm and the squall, and why nothing below is keyed on the weather name.
SPRAY = "--no-spray" not in sys.argv
SPRAY_LOG = "--spray-log" in sys.argv
SPRAY_CAP = 14_000

SPRAY_U0 = 1.6               # m/s; below bare steerage way a bow throws nothing
SPRAY_RATE = 2600.0          # droplets/s per (m/s)^2 over U0, at full burial
SPRAY_FLOOR = 0.20           # ...of that in flat water: a bow wave always spits
SPRAY_Z0, SPRAY_Z1 = 0.22, 0.46      # the shoulder arc, in lengths from midships
SPRAY_IMM0, SPRAY_IMM1 = 0.06, 0.30  # forefoot burial: dead band, then the ramp
# Of every droplet the sheet throws, this fraction belongs to the ROOT: small,
# slow, pinned to the water and dead inside half a second. The rest is the torn
# tail that arcs and blows away. A count split, not a volume one -- the tail is
# a handful of per cent of the water and most of what the eye follows, and the
# root is where all the whiteness lives.
SPRAY_ROOT_F = 0.90
# ...and the root is emitted at several times the gated rate, because it is
# gone in a tenth of a second: a population's standing count is birth rate TIMES
# life, and a sheet that dies where it is born needs to be born fast to be a
# sheet at all. The gate itself (u^2 and the forefoot burial) is untouched --
# this multiplies both ends of it, so the calm still gets the calm's trickle.
#
# 4c: cut from 4.0. The droplet layer used to BE the spray and is now the MIST
# behind the sheet -- the fine stuff hanging off the back of a parcel, which is
# a minority accent and not the subject. The sheet (SPRITE_GAIN, below) carries
# the water now, and leaving this at 4.0 put a snow flurry in front of it.
SPRAY_EMIT_GAIN = 0.9
# Quadratic air drag written per RADIUS: a = -(C/r) |v_rel| v_rel, so terminal
# velocity is sqrt(g r / C) -- 9.0 m/s for a 5 mm drop, 18 m/s for a 20 mm slam
# chunk, 5.4 m/s for a 1.5 mm one. That spread is not a detail, it IS the look:
# the fine stuff loses its own velocity in a few metres and goes wherever the
# wind goes, and the heavy stuff flies straight through the same air.
SPRAY_DRAG = 6.0e-4
SPRAY_G = 9.81
SPRAY_SLAM_V = 1.10          # m/s of closing speed before a bow entry is a slam
SPRAY_SLAM_CD = 0.22         # s; one burst per entry, not one per frame
# Death grid: 8 x 8 over 30 m of sea around her, bilinear. 64 mirror lookups a
# frame, instead of one sample_height call per droplet per frame.
SPRAY_GRID_N = 8
SPRAY_GRID_HALF = 15.0
SPRAY_FOAM_MAX = 12          # + the sheet's 4 and the two hand-placed stamps

_sp_pos = np.zeros((SPRAY_CAP, 3), np.float32)
_sp_vel = np.zeros((SPRAY_CAP, 3), np.float32)
_sp_rad = np.zeros(SPRAY_CAP, np.float32)
_sp_age = np.zeros(SPRAY_CAP, np.float32)
_sp_life = np.zeros(SPRAY_CAP, np.float32)          # <= 0 means the slot is free
_sp_buf = np.full((SPRAY_CAP, 4), -1.0, np.float32)  # what submit() reads
_sp_hi = 0                   # live prefix: no slot at or above this is alive
_sp_rng = np.random.default_rng(20260824)
_sp_grid = np.zeros((SPRAY_GRID_N, SPRAY_GRID_N), np.float32)
_sp_g0 = [0.0, 0.0]
_sp_gstep = 2.0 * SPRAY_GRID_HALF / (SPRAY_GRID_N - 1)
_sp_ginv = 1.0 / _sp_gstep
_sp_clear = None             # bow clearance last frame, for the d/dt that slams
_sp_slam_cd = 0.0
_sp_log = [0.0, 0, 0, 0.0]   # t, frames, live, ms
_sp_len = [0]                # submitted prefix, last few frames (see the submit)

# Ownership.HostRing: the positions come from the sim above, one (n, 4) memcpy a
# frame (14k droplets = 224 KB, i.e. nothing). w is the radius, which is where
# the size spread survives the trip to the GPU.
#
# STABLE SLOTS. The pool is never compacted -- allocation takes the lowest FREE
# slots and a droplet keeps its index for its whole life -- so index i names the
# same droplet in every submit, and `host_stable_slots` is the field telling the
# renderer so. Two things ride on that promise. bright_jitter is hashed on the
# slot index, so a compaction would renumber every live droplet each frame and
# the whole field would twinkle at 60 Hz. And the previous ring slot becomes a
# real prevPositions buffer, which is what the velocity STRETCH needs: without
# it (pos - prevPos) is the gap between two unrelated droplets and the field
# smears in every direction at once. With it, the sprite is a SHUTTER STREAK --
# a sheet of water leaving a bow at 8 m/s does not photograph as dots, and the
# first attempt at this section (round, uniform, no motion cue) read as a snow
# flurry for exactly that reason.
#
# Still inert here: the shader re-derives AGE from the emitter's closed form,
# which a host field has none of, so fade_power and size_taper do nothing -- the
# fade-in and the end-of-life shrink are folded into w below instead.
_pc = tp.ParticleField.Config()
_pc.capacity = SPRAY_CAP
_pc.ownership = tp.ParticleField.Ownership.HostRing
_pc.w_semantic = tp.ParticleField.WSemantic.Radius
_pc.uniform_radius = 0.004
_pc.host_stable_slots = True
spray = tp.ParticleField.create(_pc)
spray.frustum_culled = False
# Near-white with a blue-grey cool end, hard-cored and un-bloomed. The billboard
# slice is additive and composited after the upscalers, so the discipline is to
# keep the intensity LOW: spray that outshines the foam it lands in reads as
# sparks. `size_scale` is the honest sub-grid admission -- one billboard is a
# PARCEL of atomised water, not one 4 mm drop, because a 4 mm drop at 7 m is a
# third of a pixel and 14k of them is a far thinner cloud than a real sheet.
SPRAY_BB_BASE = 0.050        # 4c: halved -- these are the mist, not the sheet
spray.set_billboard_repr(tp.Color(0.93, 0.96, 1.00), tp.Color(0.62, 0.71, 0.82),
                         SPRAY_BB_BASE, 1.7)
_sb = spray.billboard_repr
_sb.lod_near = 0.0
_sb.lod_fade = 0.0
# A cine shutter, near enough: 1/48 s of travel is what a 24 fps frame of this
# would hold, and the quad is built symmetric about the particle so the streak
# comes out at about twice that. Half the shutter is therefore the honest
# number, and it is the difference between spray and hail.
_sb.stretch_seconds = 0.030
# The two caps are in different domains ON PURPOSE. stretch_max is in RADII, so
# it bounds the streak in metres and would gag the small fast droplets that are
# most of the sheet -- hence a loose one. stretch_max_screen is the one that
# does the work: 5% of the frame height, so nothing near the lens can draw a bar
# across the picture however fast it is going.
_sb.stretch_max = 40.0
_sb.stretch_max_screen = 0.05
_sb.softness = 0.30          # a droplet is a hard little thing, not a glow
_sb.fade_power = 0.0
_sb.size_taper = 0.0
_sb.bright_jitter = 0.85     # the scintillation a sunlit sheet actually has
_sb.near_fade = 0.30
_sb.glow = 0.0
spray.set_live_count(0)
scene.add(spray)


# ---- the SHEET: few, large, textured, lit, alpha-blended -----------------------
#  The dot pool above is the mist. This is the water.
#
#  A 4 mm droplet at 8 m is a third of a pixel, so fourteen thousand of them are
#  a faint speckle however they are tuned -- the first attempt at this section
#  drew exactly that and it read as a snow flurry. What a bow actually throws is
#  a SHEET: torn, metre-scale parcels of aerated water that are opaque enough to
#  hide the topsides behind them, catch the sun on their sunward face and go grey
#  on the other. Three properties do that and none of them is a dot:
#
#    * ALPHA-OVER, not additive. An additive sprite can only add light, so a
#      white puff over a bright sky is invisible and the same puff over a dark
#      hull is a lamp. Water OCCLUDES.
#    * TEXTURED. A radial falloff is a ball. A spray parcel has a ragged
#      silhouette and internal structure, which is what the fbm atlas below is.
#    * LIT by the scene's own sun, through one Henyey-Greenstein lobe. The
#      forward flare when the lens looks through the sheet into the sun is the
#      single loudest cue that it is water in air.
#
#  ONE FIELD PER VARIANT, because a field carries one texture: three fields of
#  ~200 sprites rather than one field with a per-particle atlas index, which
#  would be engine work for no visual gain at this count. The engine hashes a
#  rotation per slot on top, so three shapes do not tile.
SPRITE_N = 3                 # atlas variants == fields
SPRITE_BLK = 224             # sprites per field
SPRITE_CAP = SPRITE_N * SPRITE_BLK
# Sprites per second per unit of the dot layer's own gated rate. The GATE is
# untouched -- speed squared and forefoot burial, exactly as before -- so the
# calm reach still gets a calm reach's share of this: 30/s of dots is 0.6
# sprites/s is one parcel every two seconds, which is what a flat-water bow
# wave throws. The squall's 20 000/s is ~400/s and the sheet is continuous.
SPRITE_GAIN = 0.055
SPRITE_ROOT_F = 0.80         # ...of the sheet is low, dense and short-lived
SPRITE_DRAG_ROOT = 2.6       # 1/s toward the local air velocity
SPRITE_DRAG_TAIL = 1.25
# Aerated water is not water: a parcel that is half air falls slower than g, and
# what is left of its own momentum is gone in a metre. This is the honest
# lumped constant for "a torn sheet", not a droplet's ballistic gravity.
SPRITE_G = 7.2
SPRITE_FOAM_MAX = 4
# Radiance scale on the lit term. ABSOLUTE, unlike the dot layer's: the sprite
# pass applies the scene's own exposure and tone curve, so what this has to
# produce is a linear radiance in the same units the sea and the foam are shaded
# in -- get it wrong and the sheet is either a dark brown smear (too low, which
# is what the first sprite pass drew under a storm sky) or a paper cut-out.
SPRITE_BASE = 1.0
# ...and the SKY's share of the light, which the billboard pass cannot sample
# (the ambience here is an env map, and there is no AmbientLight in the scene).
# Driven per frame off the same daylight the rest of the grading uses: under a
# black squall almost all the light on a sheet of spray is this, not the sun.
SPRITE_SKY = 1.35


def _spray_puff(rng, n=80):
    """One greyscale spray-puff alpha mask: value-noise fbm inside a ragged
    radial envelope. RGB is flat white -- the field's own colour and the sun
    tint it -- so only the alpha channel carries shape."""
    g1 = np.linspace(-1.0, 1.0, n, dtype=np.float32)
    u, v = np.meshgrid(g1, g1)
    r = np.sqrt(u * u + v * v)
    f = np.zeros((n, n), np.float32)
    amp, res = 1.0, 3
    for _ in range(4):
        g = rng.random((res + 1, res + 1)).astype(np.float32)
        t = np.linspace(0.0, res, n, dtype=np.float32)
        i0 = np.minimum(np.floor(t).astype(np.intp), res - 1)
        w = (t - i0)[:, None]
        row = g[i0] * (1.0 - w) + g[i0 + 1] * w          # (n, res+1)
        w2 = (t - i0)[None, :]
        f += amp * (row[:, i0] * (1.0 - w2) + row[:, i0 + 1] * w2)
        amp *= 0.55
        res *= 2
    f = (f - f.min()) / max(float(f.max() - f.min()), 1e-6)
    # The envelope is the fbm's own low end, so the silhouette tears where the
    # inside is thin -- a torn sheet, not a disc with noise painted on it.
    edge = 0.30 + 0.62 * f
    a = np.clip((1.0 - r / edge) * 2.4, 0.0, 1.0) ** 1.25 * (0.42 + 0.58 * f)
    # Hard zero before the quad's own rim: the engine rotates the LOOKUP per
    # particle, and anything still opaque at |uv| = 1 would wrap.
    a *= np.clip((0.97 - r) * 7.0, 0.0, 1.0)
    img = np.empty((n, n, 4), np.uint8)
    img[..., 0:3] = 255
    img[..., 3] = (np.clip(a, 0.0, 1.0) * 255.0 + 0.5).astype(np.uint8)
    return img


_spr_pos = np.zeros((SPRITE_CAP, 3), np.float32)
_spr_vel = np.zeros((SPRITE_CAP, 3), np.float32)
_spr_rad = np.zeros(SPRITE_CAP, np.float32)
_spr_age = np.zeros(SPRITE_CAP, np.float32)
_spr_life = np.zeros(SPRITE_CAP, np.float32)
_spr_kd = np.zeros(SPRITE_CAP, np.float32)
_spr_buf = np.full((SPRITE_CAP, 4), -1.0, np.float32)
_spr_hi = [0] * SPRITE_N
_spr_next = [0]              # round-robin start, so the three fields fill evenly
sprites = []
if SPRAY:
    _spr_rng = np.random.default_rng(20260824 + 7)
    for _v in range(SPRITE_N):
        _sc = tp.ParticleField.Config()
        _sc.capacity = SPRITE_BLK
        _sc.ownership = tp.ParticleField.Ownership.HostRing
        _sc.w_semantic = tp.ParticleField.WSemantic.Radius
        _sc.uniform_radius = 0.30
        # Stable slots, and therefore NO back-to-front sort: the engine draws
        # instances in slot order, so sorting would mean renumbering, and
        # renumbering costs the velocity stretch (which needs slot i to be the
        # same parcel across submits) and the per-slot rotation hash. Between a
        # streaking, non-tiling sheet with imperfect intra-field blend order and
        # a correctly ordered field of round untextured blobs, the streaks win:
        # a few hundred sprites of one colour and one opacity are forgiving
        # about order in a way that a smoke plume would not be.
        _sc.host_stable_slots = True
        _f = tp.ParticleField.create(_sc)
        _f.frustum_culled = False
        _f.set_billboard_repr(tp.Color(0.94, 0.97, 1.00), tp.Color(0.70, 0.78, 0.88),
                              SPRITE_BASE, 1.0)
        _b = _f.billboard_repr
        _b.texture = tp.data_texture(_spray_puff(_spr_rng), False)
        _b.alpha_over = True
        _b.lit = True
        # Forward-ish, but only just: a spray parcel is a cloud of spheres, and
        # its phase function is nowhere near as peaked as a single drop's.
        _b.lit_phase_g = 0.36
        # The sky is an env map this pass cannot sample and the scene carries no
        # AmbientLight, so this is the shaded side's floor. Low: a sheet with
        # its back to the sun IS nearly grey.
        _b.lit_ambient = 0.55
        _b.opacity = 0.82
        # A parcel moving 6 m/s across the frame is 10 cm of travel in a frame
        # and its own diameter besides, so the streak is a modest one -- the
        # sheet reads as MOVING, not as motion-blurred to mush.
        _b.stretch_seconds = 0.018
        _b.stretch_max = 3.0
        _b.stretch_max_screen = 0.09
        _b.bright_jitter = 0.30
        _b.fade_power = 0.0
        _b.size_taper = 0.0
        # A metre-wide sprite half a metre from the lens is the whole frame.
        _b.near_fade = 0.9
        _b.glow = 0.0
        _f.set_live_count(0)
        scene.add(_f)
...
def _sprite_alloc(n):
    """`n` free sprite slots, spread across the three fields round-robin so no
    one variant carries the sheet. Returns a flat index array into the pool."""
    if n <= 0:
        return np.empty(0, np.intp)
    out = []
    per = max(1, (n + SPRITE_N - 1) // SPRITE_N)
    for j in range(SPRITE_N):
        v = (_spr_next[0] + j) % SPRITE_N
        b = v * SPRITE_BLK
        free = np.flatnonzero(_spr_life[b:b + SPRITE_BLK] <= 0.0)[:per]
        if free.size:
            out.append(free + b)
        if sum(a.size for a in out) >= n:
            break
    _spr_next[0] = (_spr_next[0] + 1) % SPRITE_N
    return np.concatenate(out)[:n] if out else np.empty(0, np.intp)


def _sprite_spawn(k, pos, vel, rad, life, kd):
    _spr_pos[k] = pos
    _spr_vel[k] = vel
    _spr_rad[k] = rad
    _spr_age[k] = 0.0
    _spr_life[k] = life
    _spr_kd[k] = kd
    for v in range(SPRITE_N):
        b = v * SPRITE_BLK
        m = k[(k >= b) & (k < b + SPRITE_BLK)]
        if m.size:
            _spr_hi[v] = max(_spr_hi[v], int(m.max()) - b + 1)


def _spray_grid(cx, cz):
    """Refresh the coarse sea-height grid around her. 64 CPU mirror lookups."""
    x0, z0 = cx - SPRAY_GRID_HALF, cz - SPRAY_GRID_HALF
    _sp_g0[0], _sp_g0[1] = x0, z0
    for j in range(SPRAY_GRID_N):
        z = z0 + j * _sp_gstep
        for i in range(SPRAY_GRID_N):
            _sp_grid[j, i] = ocean.sample_height(x0 + i * _sp_gstep, z)


def _spray_sea(px, pz):
    """Bilinear sea height under every droplet, out of the coarse grid."""
    fx = np.clip((px - _sp_g0[0]) * _sp_ginv, 0.0, SPRAY_GRID_N - 1.001)
    fz = np.clip((pz - _sp_g0[1]) * _sp_ginv, 0.0, SPRAY_GRID_N - 1.001)
    i0, j0 = fx.astype(np.intp), fz.astype(np.intp)
    tx, tz = (fx - i0), (fz - j0)
    g = _sp_grid
    a = g[j0, i0] + (g[j0, i0 + 1] - g[j0, i0]) * tx
    b = g[j0 + 1, i0] + (g[j0 + 1, i0 + 1] - g[j0 + 1, i0]) * tx
    return a + (b - a) * tz


def _spray_alloc(n):
    """`n` free slots, LOWEST FIRST -- see the note on bright_jitter above.

    Bounded scan: everything at or above the live prefix is free by definition,
    so hi + n is always enough to find n of them and a calm frame never touches
    the other 13,900 slots."""
    lim = min(SPRAY_CAP, _sp_hi + n + 32)
    return np.flatnonzero(_sp_life[:lim] <= 0.0)[:n]


def _spray_spawn(k, pos, vel, rad, life):
    """Write one batch of droplets into the pool and open up the live prefix."""
    global _sp_hi
    _sp_pos[k] = pos
    _sp_vel[k] = vel
    _sp_rad[k] = rad
    _sp_age[k] = 0.0
    _sp_life[k] = life
    _sp_hi = max(_sp_hi, int(k[-1]) + 1)


def _spray_log(dt, u, clear, sea, rate, t0, live=0):
    """`--spray-log`: one line a second of what the two gates are actually
    doing. This is the tuning instrument -- the whole claim of this section is
    that ONE set of numbers gives a calm reach a trickle and the squall sheets,
    and `sea` / `rate` / `live` is where that claim is either true or not."""
    if not SPRAY_LOG:
        return
    _sp_log[0] += dt
    _sp_log[1] += 1
    _sp_log[2] += live
    _sp_log[3] += (time.perf_counter() - t0) * 1e3
    if _sp_log[0] >= 1.0:
        n = max(_sp_log[1], 1)
        print(f"[spray] u {u:4.2f}  clear {clear:+6.3f}  sea {sea:4.2f}  "
              f"rate {rate:7.1f}/s  live {_sp_log[2] // n:6d}  hi {_sp_hi:6d}  "
              f"{_sp_log[3] / n:5.3f} ms/f")
        _sp_log[0] = _sp_log[3] = 0.0
        _sp_log[1] = _sp_log[2] = 0


def _sprite_sheet(n, org, rot, hull_vel, u, sea, roll):
    """Emit `n` sheet parcels off the bow shoulders. The GEOMETRY is attempt
    one's, which passed: the shoulder station, the lee-side weighting off heel,
    the outward-and-slightly-up deflection fan. Only the SCALE changes -- a
    parcel is 20 cm to a metre across and stands in for a few thousand of the
    droplets the dot layer draws individually."""
    k = _sprite_alloc(n)
    m = k.size
    if not m:
        return
    rnd = _spr_rng.random
    root = rnd(m) < SPRITE_ROOT_F
    zc = SPRAY_Z0 + (SPRAY_Z1 - SPRAY_Z0) * rnd()
    t = np.clip(zc + np.where(root, 0.150, 0.075) * _spr_rng.standard_normal(m),
                SPRAY_Z0, SPRAY_Z1).astype(np.float32)
    zl = LOA * t
    tt = np.clip(zl / HULL_HALF_L, 0.0, 0.995)
    hb = HULL_HALF_B * (1.0 - tt * tt) ** 0.55
    lee = -1.0 if roll > 0.0 else 1.0
    w_lee = 0.5 + 0.42 * min(abs(roll) / 0.35, 1.0)
    sd = np.where(rnd(m) < w_lee, lee, -lee).astype(np.float32)
    loc = np.stack([sd * hb * (0.95 + 0.35 * rnd(m)), 0.02 + 0.16 * rnd(m), zl],
                   1).astype(np.float32)
    ax = 0.65 + 0.85 * rnd(m)
    ay = np.where(root, 0.06 + 0.24 * rnd(m), 0.28 + 0.60 * rnd(m))
    az = 0.05 + 0.50 * rnd(m)
    d = np.stack([sd * ax, ay, az], 1).astype(np.float32)
    d /= np.linalg.norm(d, axis=1, keepdims=True)
    sf = np.where(root, 0.18 + 0.34 * rnd(m), 0.42 + 0.70 * rnd(m))
    spd = (u * sf * (0.60 + 0.80 * sea)).astype(np.float32)
    # The root parcels are the dense white foot of the sheet and the tail is
    # what tears off the top of it, so they differ by about an octave in size
    # as well as in life -- the same split the droplet layer makes, one scale up.
    rad = np.where(root,
                   0.14 + 0.24 * rnd(m),
                   0.30 + 0.55 * rnd(m)).astype(np.float32)
    pos = org + loc @ rot.T
    # Pinned to the WATER, not to the deck: she heaves half a metre in the
    # squall and a sheet that rode up with her would float.
    py = _spray_sea(pos[:, 0], pos[:, 2]) + 0.02 + 0.15 * rnd(m)
    pos[:, 1] = np.where(root, py, pos[:, 1])
    _sprite_spawn(k, pos,
                  hull_vel * (0.50 + 0.22 * root.astype(np.float32))[:, None]
                  + d * spd[:, None] @ rot.T,
                  rad,
                  np.where(root, 0.26 + 0.30 * rnd(m),
                           0.55 + 0.70 * rnd(m)).astype(np.float32),
                  np.where(root, SPRITE_DRAG_ROOT, SPRITE_DRAG_TAIL).astype(np.float32))


def _sprite_slam(n, org, rot, hull_vel, hit):
    """The chunks a buried forefoot throws: bigger, slower to disperse, and up
    rather than out. This is the moment the film's squall shots are for."""
    k = _sprite_alloc(n)
    m = k.size
    if not m:
        return
    rnd = _spr_rng.random
    zl = LOA * 0.44 + 0.7 * (rnd(m) - 0.5)
    tt = np.clip(zl / HULL_HALF_L, 0.0, 0.995)
    hb = HULL_HALF_B * (1.0 - tt * tt) ** 0.55
    sd = np.where(rnd(m) < 0.5, 1.0, -1.0).astype(np.float32)
    loc = np.stack([sd * hb * (0.4 + 0.7 * rnd(m)),
                    0.05 + 0.30 * rnd(m), zl], 1).astype(np.float32)
    d = np.stack([sd * (0.20 + 1.00 * rnd(m)),
                  0.80 + 0.90 * rnd(m),
                  0.20 + 0.80 * rnd(m)], 1).astype(np.float32)
    d /= np.linalg.norm(d, axis=1, keepdims=True)
    spd = (hit * (0.45 + 0.65 * rnd(m))).astype(np.float32)
    _sprite_spawn(k, org + loc @ rot.T,
                  hull_vel * 0.45 + d * spd[:, None] @ rot.T,
                  (0.40 + 0.62 * rnd(m)).astype(np.float32),
                  (0.85 + 0.75 * rnd(m)).astype(np.float32),
                  np.float32(1.5))


def _sprite_step(dt, wind):
    """Fly the sheet, kill it at the sea, and submit one field per variant."""
    stamps = 0
    for v in range(SPRITE_N):
        hi = _spr_hi[v]
        b = v * SPRITE_BLK
        if hi == 0:
            sprites[v].set_live_count(0)
            continue
        sl = slice(b, b + hi)
        al = _spr_life[sl] > 0.0
        af = al.astype(np.float32)[:, None]
        p, vel = _spr_pos[sl], _spr_vel[sl]
        # Relaxation toward the air rather than a quadratic drag law: a torn
        # parcel has no single radius and its own momentum is gone in a metre,
        # which is a time constant, not a Reynolds number.
        vel -= (vel - wind.astype(np.float32)) * (_spr_kd[sl][:, None] * dt) * af
        vel[:, 1] -= (SPRITE_G * dt) * af[:, 0]
        p += vel * dt
        _spr_age[sl] += dt * af[:, 0]

        sunk = al & (p[:, 1] < _spray_sea(p[:, 0], p[:, 2]))
        si = np.flatnonzero(sunk)
        for j in si[:max(0, SPRITE_FOAM_MAX - stamps)]:
            stamps += 1
            r = float(_spr_rad[b + j])
            ocean.add_foam_disturbance(float(p[j, 0]), float(p[j, 2]),
                                       1.1 + 2.6 * r, min(0.16 + 0.55 * r, 0.55))
        dead = sunk | (al & (_spr_age[sl] >= _spr_life[sl]))
        if dead.any():
            _spr_life[sl][dead] = 0.0
            _spr_vel[sl][dead] = 0.0
            al = al & ~dead

        # w carries the WHOLE lifecycle, because a HostRing field has no age
        # channel and the shader's age-driven fade is therefore inert on it. So
        # the parcel blooms out of the bow over the first tenth of its life,
        # spreads as it goes (torn water disperses), and collapses over the last
        # third. Collapsing rather than fading is the honest limitation of one
        # float per particle: alpha per sprite would need a second channel, and
        # with a feathered sprite the two read nearly the same at this size.
        ag = _spr_age[sl]
        lf = np.maximum(_spr_life[sl], 1e-3)
        uu = np.clip(ag / lf, 0.0, 1.0)
        grow = np.minimum(uu * 10.0, 1.0)
        tail = np.clip((1.0 - uu) / 0.34, 0.0, 1.0) ** 0.7
        w = _spr_rad[sl] * (0.36 + 0.64 * grow) * (1.0 + 0.55 * uu) * tail
        _spr_buf[sl, 0:3] = p
        _spr_buf[sl, 3] = np.where(al, w, -1.0)
        nz = np.flatnonzero(al)
        _spr_hi[v] = int(nz[-1]) + 1 if nz.size else 0
        sprites[v].submit(_spr_buf[b:b + hi], dt)


def spray_step(dt, rot, org, hull_vel, u, roll, wind):
    """One step of the droplet sim, and one submit. Called from step_boat with
    the SAME pose the wake and the foam stamps are built from."""
    global _sp_hi, _sp_clear, _sp_slam_cd
    if not SPRAY or dt <= 0.0 or not first_render_done:
        return
    t_spray = time.perf_counter() if SPRAY_LOG else 0.0
    bs = boat_state
    _spray_grid(bs["x"], bs["z"])

    # ---- what the sea is doing to the forefoot ------------------------------
    # One exact sample at the bow rather than the coarse grid: this drives the
    # slam trigger, and a 4 m grid cell would smear the very event it is for.
    bow = org + rot @ np.array([0.0, 0.0, HULL_HALF_L * 0.94])
    clear = float(bow[1] - ocean.sample_height(float(bow[0]), float(bow[2])))
    vclose = 0.0 if _sp_clear is None else (clear - _sp_clear) / dt
    _sp_clear = clear
    # `sea` is the whole difference between the calm and the squall. The dead
    # band is her own 2 cm trim draft plus slop, so flat water reads exactly
    # zero and the noon reach falls back on the 0.10 floor in the rate.
    sea = min(max(-clear - SPRAY_IMM0, 0.0) / SPRAY_IMM1, 1.6)

    # ---- 1. the bow-shoulder sheet ------------------------------------------
    gate = max(u - SPRAY_U0, 0.0)
    rate = SPRAY_RATE * gate * gate * (SPRAY_FLOOR + 1.30 * sea)
    # Stochastic rounding: 0.4 droplets a frame is two every five frames, not
    # zero forever and not a 60 Hz strobe.
    n = int(rate * dt * SPRAY_EMIT_GAIN + _sp_rng.random())
    if n > 0:
        k = _spray_alloc(n)
        m = k.size
        if m:
            rnd = _sp_rng.random
            # ROOT and TAIL, one emission, two populations. A bow sheet is not
            # a cloud of droplets on one arc: it is a DENSE WHITE ROOT lying on
            # the water within a metre of the stem -- small, fast, gone in half
            # a second, and where all the whiteness is -- with a thin torn-off
            # tail of bigger drops arcing over it and blowing away. Attempt 1
            # emitted only the tail, which is why it read as a flurry of
            # unattached dots hanging in the air with nothing beneath them. The
            # root is the majority by COUNT and the minority by volume, which is
            # also true of the real thing.
            root = rnd(m) < SPRAY_ROOT_F
            rf = root.astype(np.float32)
            # One narrow band per frame rather than the whole arc every
            # frame: a bow sheet tears off in PUFFS as each wave face passes,
            # and a uniform curtain is the single biggest thing that makes a
            # particle field read as weather instead of as water. The root
            # spreads wider than the tail -- the sheet is continuous along the
            # shoulder; what tears off it is local.
            zc = SPRAY_Z0 + (SPRAY_Z1 - SPRAY_Z0) * _sp_rng.random()
            t = np.clip(zc + np.where(root, 0.105, 0.055) * _sp_rng.standard_normal(m),
                        SPRAY_Z0, SPRAY_Z1).astype(np.float32)
            zl = LOA * t
            # Half-beam at that station -- the shoulder is where the waterline
            # is still wide but already turning in, and the sheet leaves along
            # it, so the emission line has to follow the hull and not a box.
            tt = np.clip(zl / HULL_HALF_L, 0.0, 0.995)
            hb = HULL_HALF_B * (1.0 - tt * tt) ** 0.55
            # Heel decides which shoulder. Positive rotation.z lifts starboard,
            # i.e. she is heeled to PORT and the lee bow is -X; the lee shoulder
            # is the one buried in the bow wave and it throws most of the water.
            lee = -1.0 if roll > 0.0 else 1.0
            w_lee = 0.5 + 0.42 * min(abs(roll) / 0.35, 1.0)
            sd = np.where(rnd(m) < w_lee, lee, -lee).astype(np.float32)
            loc = np.stack([sd * hb, 0.02 + 0.22 * rnd(m), zl], 1).astype(np.float32)

            # Nearly horizontal, and the root more so. A sheet is water
            # DEFLECTED sideways by the bow, not water thrown up: the root of
            # it stays within a few centimetres of the surface and only the
            # torn-off fine end goes anywhere near the rail.
            ax = 0.70 + 0.90 * rnd(m)
            ay = np.where(root, 0.04 + 0.22 * rnd(m), 0.22 + 0.55 * rnd(m))
            az = 0.05 + 0.55 * rnd(m)
            d = np.stack([sd * ax, ay, az], 1).astype(np.float32)
            d /= np.linalg.norm(d, axis=1, keepdims=True)
            # The sheet is thrown by the DEFLECTION, so it keeps only about half
            # her forward momentum and gets the rest from the bow wave itself.
            sf = np.where(root, 0.20 + 0.42 * rnd(m), 0.50 + 0.85 * rnd(m))
            spd = (u * sf * (0.65 + 0.75 * sea)).astype(np.float32)
            # Lognormal radii, and the root's distribution sits a full octave
            # below the tail's. "Mostly 3-8 mm" is a distribution, not a number:
            # a single radius, or a uniform one, is what makes a particle field
            # read as confetti.
            rad = np.where(root,
                           np.clip(0.0017 * np.exp(0.55 * _sp_rng.standard_normal(m)),
                                   0.0008, 0.0038),
                           np.clip(0.0034 * np.exp(0.80 * _sp_rng.standard_normal(m)),
                                   0.0013, 0.0085)).astype(np.float32)
            pos = org + loc @ rot.T
            # The root is pinned to the WATER, not to the hull: she heaves half
            # a metre in the squall and a sheet that rode the deck up with her
            # would float. This is the one thing that makes it read as rooted.
            py = _spray_sea(pos[:, 0], pos[:, 2]) + 0.010 + 0.085 * rnd(m)
            pos[:, 1] = np.where(root, py, pos[:, 1])
            _spray_spawn(k, pos,
                         hull_vel * (0.55 + 0.20 * rf)[:, None]
                         + d * spd[:, None] @ rot.T,
                         rad,
                         # Nothing HANGS. The tail's ceiling is a second, not
                         # three: a droplet whose own velocity drag has eaten
                         # goes wherever the air goes and stops being water in
                         # flight -- it becomes a dot suspended over the sea,
                         # which is the exact texture of a snow flurry.
                         np.where(root, 0.14 + 0.34 * rnd(m),
                                  0.45 + 0.65 * rnd(m)).astype(np.float32))

    # ...and the SHEET, off the same gate and the same pose. Capped per frame so
    # a 120 fps frame and a 30 fps one throw the same water per second without
    # either being able to flood the pool in one step.
    if sprites:
        ns = int(rate * dt * SPRITE_GAIN + _spr_rng.random())
        if ns > 0:
            _sprite_sheet(min(ns, 20), org, rot, hull_vel, u, sea, roll)

    # ---- 2. slam bursts: burying the bow ------------------------------------
    _sp_slam_cd = max(0.0, _sp_slam_cd - dt)
    if (vclose < -SPRAY_SLAM_V and clear < 0.22 and _sp_slam_cd <= 0.0
            and u > 1.2):
        _sp_slam_cd = SPRAY_SLAM_CD
        hit = min(-vclose, 7.0)
        k = _spray_alloc(min(int(48.0 * (hit - 1.0) ** 2), 900))
        m = k.size
        if m:
            rnd = _sp_rng.random
            zl = LOA * 0.44 + 0.55 * (rnd(m) - 0.5)
            tt = np.clip(zl / HULL_HALF_L, 0.0, 0.995)
            hb = HULL_HALF_B * (1.0 - tt * tt) ** 0.55
            sd = np.where(rnd(m) < 0.5, 1.0, -1.0).astype(np.float32)
            loc = np.stack([sd * hb * (0.4 + 0.7 * rnd(m)),
                            0.05 + 0.30 * rnd(m), zl], 1).astype(np.float32)
            d = np.stack([sd * (0.20 + 1.10 * rnd(m)),
                          0.70 + 0.90 * rnd(m),
                          0.20 + 0.90 * rnd(m)], 1).astype(np.float32)
            d /= np.linalg.norm(d, axis=1, keepdims=True)
            spd = (hit * (0.55 + 0.80 * rnd(m))).astype(np.float32)
            # A slam is not the sheet with the gain up: it throws CHUNKS, whole
            # centimetre-scale lumps of green water that fly nearly ballistically
            # while the fine half of the same burst is gone in three metres.
            big = rnd(m) < 0.42
            rad = np.where(big, 0.015 + 0.011 * rnd(m),
                           0.0022 + 0.0045 * rnd(m)).astype(np.float32)
            _spray_spawn(k, org + loc @ rot.T,
                         hull_vel * 0.45 + d * spd[:, None] @ rot.T,
                         rad, (1.4 + 1.8 * rnd(m)).astype(np.float32))
        if sprites:
            _sprite_slam(min(int(3 + 2.4 * (hit - 1.0) ** 2), 22),
                         org, rot, hull_vel, hit)

    # The sheet flies on its own clock and its own pool, and it must step even
    # on a frame where the droplet pool is empty -- hence before the early
    # return below.
    if sprites:
        _sprite_step(dt, wind)

    hi = _sp_hi
    if hi == 0:
        spray.set_live_count(0)
        _spray_log(dt, u, clear, sea, rate, t_spray)
        return

    # ---- 3. fly them --------------------------------------------------------
    al = (_sp_life[:hi] > 0.0)
    af = al.astype(np.float32)[:, None]
    v, p = _sp_vel[:hi], _sp_pos[:hi]
    vr = v - wind.astype(np.float32)      # the air moves; the droplet feels that
    kd = (SPRAY_DRAG / _sp_rad[:hi]) * np.sqrt((vr * vr).sum(1))
    v -= vr * (kd[:, None] * dt) * af
    v[:, 1] -= (SPRAY_G * dt) * af[:, 0]
    p += v * dt
    _sp_age[:hi] += dt * af[:, 0]

    # ---- 4. death, and the foam it leaves -----------------------------------
    sunk = al & (p[:, 1] < _spray_sea(p[:, 0], p[:, 2]))
    dead = sunk | (al & (_sp_age[:hi] >= _sp_life[:hi]))
    si = np.flatnonzero(sunk)
    if si.size:
        # Merged into the same clear + re-add block the hull stamps use, and
        # capped so the two of them together stay under the engine's 64.
        if si.size > SPRAY_FOAM_MAX:
            si = si[np.linspace(0, si.size - 1, SPRAY_FOAM_MAX).astype(np.intp)]
        for j in si:
            r = float(_sp_rad[j])
            ocean.add_foam_disturbance(float(p[j, 0]), float(p[j, 2]),
                                       0.50 + 16.0 * r, min(0.09 + 9.0 * r, 0.50))
    if dead.any():
        _sp_life[:hi][dead] = 0.0
        _sp_vel[:hi][dead] = 0.0
        al = al & ~dead

    # ---- 5. one memcpy ------------------------------------------------------
    # w carries the radius AND the whole visual lifecycle: fade-in over the
    # first 50 ms (a droplet must not pop into existence at full size on the
    # hull), shrink over the last fifth of life, and -1 for a free slot, which
    # the vertex stage collapses to a zero-area quad for free.
    age, life = _sp_age[:hi], np.maximum(_sp_life[:hi], 1e-3)
    grow = np.minimum(age * 20.0, 1.0)
    rem = np.clip((1.0 - age / life) * 5.0, 0.0, 1.0)
    _sp_buf[:hi, 0:3] = p
    _sp_buf[:hi, 3] = np.where(al, _sp_rad[:hi] * grow * (0.30 + 0.70 * rem), -1.0)
    nz = np.flatnonzero(al)
    _sp_hi = int(nz[-1]) + 1 if nz.size else 0
    # The submitted PREFIX never shrinks faster than the ring is deep. Under
    # host_stable_slots the renderer reads the previous submit as this one's
    # previous step, and a slot beyond the previous submit's length holds
    # whatever was there three frames ago -- harmless (the stretch is capped)
    # but it is free to avoid: hold the length at the recent maximum and those
    # slots are submitted dead (w < 0) instead of left stale. Costs the memcpy
    # a few hundred rows, and only while the pool is draining.
    _sp_len.append(_sp_hi)
    del _sp_len[:-4]
    nsub = max(_sp_len)
    if nsub:
        # dt is what makes stretch_seconds a SHUTTER rather than a per-frame
        # smear: the streak has to be the same length at 30 fps and at 120.
        spray.submit(_sp_buf[:nsub], dt)
    else:
        spray.set_live_count(0)

    _spray_log(dt, u, clear, sea, rate, t_spray, int(nz.size))
...
def step_boat(dt):
    ...
    # ... and she throws water into the AIR. Same pose the wake and the stamps
    # above were built from, and the droplets that land add their own stamps to
    # the very list this block just opened -- so a splash rejoins the sea in the
    # same frame it hits it.
    spray_step(dt, rot, np.array([bs["x"], bs["y"], bs["z"]]), boat_vel,
               bs["u"], roll, true_wind)
...
def update_world(dt):
    ...
    # ---- spray -------------------------------------------------------------
    # Same story as the rain: additive quads composited after the upscalers and
    # never touched by auto exposure, so the intensity has to walk against the
    # exposure the scene is metering at, or spray that reads right at noon is a
    # constellation at dusk. Held down harder than the rain -- lit water is
    # bright but it is not a light source.
    if SPRAY:
        # ...and the quads are UNLIT, so the one thing a real sheet does that
        # they cannot -- flare when the camera looks through it into the sun and
        # go grey when the sun is behind the lens -- is applied here as a
        # frame-uniform gain. Not lighting: a Mie forward lobe evaluated once
        # for the whole field from the sun and the view direction, which is
        # the honest reach of a per-frame scalar. Backscatter gets its own
        # small term because a sunlit sheet seen down-sun is bright too, just
        # not like this.
        fw = camera.get_world_direction()
        # +1 = the lens is pointed at the sun and the sheet is lit from behind.
        cosp = fw.x * cs.sun_dir[0] + fw.y * cs.sun_dir[1] + fw.z * cs.sun_dir[2]
        phase = 0.62 + 1.55 * max(cosp, 0.0) ** 3 + 0.30 * max(-cosp, 0.0) ** 2
        spray.billboard_repr.intensity = (
                SPRAY_BB_BASE * (1.0 + 2.2 * (1.0 - cs.daylight) ** 2) *
                (1.0 + (phase - 1.0) * cs.daylight))
        # The SHEET needs none of that -- the engine lights it from the scene's
        # own sun through a real phase function, and the pass applies the
        # scene's exposure. The one thing it cannot know is the SKY, which is an
        # env map it has no descriptor for, so the demo supplies the ambient
        # floor: nearly all the light on a sheet under a black squall.
        for _sf in sprites:
            _sf.billboard_repr.lit_ambient = SPRITE_SKY * (0.10 + 0.90 * cs.daylight)
...
SHORT_CUT = [
    ...
    ("morning: FPV, the same move", 3.0,
     dict(name="drone sensors: FPV rgb", du=(0.42, 0.594), caption="drone Â· rgb")),
    ("morning: FPV, the same move", 3.5,
     dict(name="drone sensors: depth", du=(0.594, 0.797), sensor="depth",
          caption=f"drone Â· depth Â· {DEPTH_NEAR:.0f}-{DEPTH_FAR:.0f} m")),
    ("morning: FPV, the same move", 3.5,
     dict(name="drone sensors: event camera", du=(0.797, 1.0), sensor="events",
          caption=f"drone Â· event camera Â· {EVT_W}x{EVT_H} DVS")),
...
if FILM:
    ...
    if SEQ and strike_frame > 0:
        ...
        # No strike: write the last few frames of the shot instead, 30 frames
        # (half a second) apart. Anything that has to be judged IN MOTION --
        # spray, wake, the sheet tearing off the bow -- has to be caught on a
        # LIVE frame, and the `--out` still is taken after 24 settle ticks that
        # do not step the boat, so a sim driven from frame() is frozen in it.
        for o in (90, 60, 30, 1):
            if frames - o > 0:
                seq[frames - o] = f"{SEQ}_f{frames - o:04d}.png"
    ...
    for f in range(frames):
        ...
        if FACE in ("sun", "anti"):
            ...
            # The hull-spray check: a chase boat's lens four metres off the lee
            # bow, a hand's breadth above the water, looking back along the
            # shoulder where the sheet leaves. This is the ONE framing that can
            # judge it -- the film's own waterline shot is amidships and its
            # squall shots are wide -- and it is deliberately weather-driven
            # rather than scripted, so the same command with `--weather clear`
            # and `--weather squall` is the whole calm-vs-storm gate.
            bs_ = boat_state
            cy, sy = math.cos(bs_["yaw"]), math.sin(bs_["yaw"])
            lee = -1.0 if bs_["heel"] > 0.0 else 1.0
            # 4.5 m abeam on the lee side, four metres ahead of the stem,
            # 0.9 m up, looking back down the shoulder.
            ox, oz = lee * 3.0, 6.4
            camera.position.set(bx + cy * ox + sy * oz, bs_["y"] + 0.70,
                                bz - sy * ox + cy * oz)
            camera.look_at(bx + sy * (LOA * 0.30) + cy * ox * 0.30,
                           bs_["y"] + 0.10,
                           bz + cy * (LOA * 0.30) - sy * ox * 0.30)
        elif FACE == "under":
