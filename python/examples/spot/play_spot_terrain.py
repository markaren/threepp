...
hot-reloading the checkpoint as train_spot_terrain.py writes it. STEER it yourself and watch it track.
    ...
    python play_spot_terrain.py                     # live window; hot-reloads spot_terrain.pt
...
def v2_obs(art, last_act, rise, run, n, cmd):
    ...
    hx, hy = float(R[0, 0]), float(R[1, 0])                        # body +x axis in world (heading)
...
def main():
    ...
    def control_tick(use_keys=True):
        ...
        with torch.no_grad():
            obs = v2_obs(art, state["last_act"], built["rise"], built["run"], built["n"], cmd)
