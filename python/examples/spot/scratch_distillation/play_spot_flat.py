...
Loads scratch_flat_best.pt, runs the policy on CPU in a live threepp GL window with a
chase cam, keyboard steering, ImGui HUD, and hot-reload.
...
with normalize_obs=True; feeding raw obs gives garbage (wrong scale + units).
    ...
    python play_spot_flat.py --model scratch_flat_latest.pt
...
from _common import resolve_model, chase_cam, poll_reload
...
def main():
    ...
    ap.add_argument("--model", default=os.path.join(_HERE, "scratch_flat_best.pt"),
                    help="checkpoint path (default: scratch_flat_best.pt, the gated best and the "
                         "only one tracked in git; falls back to scratch_flat_latest.pt)")
    ...
    def frame():
        ...
        if nf[0] % 40 == 0:    # hot-reload the checkpoint ~1/s
            ...
            poll_reload(pol, model, dev, on_reload=on_reload, require_norm=True)
