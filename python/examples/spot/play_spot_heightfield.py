...
the 96-d clock layout [proprio(48)|clock(2)|base_above(1)|scan(45)], normalized by the checkpoint's
RunningNorm, with a phase clock advanced at the training rate; Spot built at the base gait's STIFF_GAINS
(90). tgs_pcm/0.005.
"""
import argparse
import math
...
def main():
    ...
    def control_tick(use_keys=True):
        ...
        ahead, h_here = scanner.scan(art.root_state()) if scanner is not None else analytic_scan(art, lambda x, y: terr_h(x, y, hf))
        with torch.no_grad():
