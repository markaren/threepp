...
def main():
    ...
    ap.add_argument("--urdf", default=str(cache / "model.urdf"))
    ap.add_argument("--out", default=str(cache / "spot_physics.urdf"))
    ap.add_argument("--collision-axis", choices=("threepp", "urdf"), default="threepp")
    args = ap.parse_args()
    if not os.path.exists(args.urdf):
        print(f"no URDF at {args.urdf} - run spot_deploy.py once to fetch the assets")
        return 1
